...
Methodology Summary
Data: SystemsLab-Sapienza pump-and-dump dataset (La Morgia et al., 2021) — 12 market-microstructure features, 3 temporal resolutions, Jan 2018–Jan 2021
Splits: 208 training-fit / 53 validation / 66 test events, no shared events across partitions
Imbalance handling: class weighting for tree ensembles/SVM, focal loss for the DNN, SMOTE evaluated for Naive Bayes
Model selection: validation-only, with a single audited test read for the winning model
Robustness checks: GroupKFold CV, chronological retrain, event-level bootstrap CIs
Tech Stack
